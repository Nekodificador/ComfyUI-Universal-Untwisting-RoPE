...
def _strength_curve_value(curve: Any, progress: float) -> float:
    """Sample a per-step strength curve at `progress` (0=first step .. 1=last).

    Accepts a list/tuple/tensor of floats (e.g. NKD Sigmas Curve output). Maps by position along
    the trajectory, like Klein's Reference Control curve (by step, not sigma). Returns 1.0 (neutral)
    if the curve is empty or unreadable, so a missing/garbage curve never changes the result.
    """
    try:
        seq = curve.detach().cpu().tolist() if hasattr(curve, 'detach') else list(curve)
    except Exception:
        return 1.0
    if not seq:
        return 1.0
    if len(seq) == 1:
        return float(seq[0])
    pos = max(0.0, min(1.0, float(progress))) * (len(seq) - 1)
    return float(seq[int(round(pos))])
...
class UntwistingRoPE:
    ...
    def patch(
        ...
        def model_function_wrapper(apply_model: Callable, args: Dict[str, Any]) -> torch.Tensor:
            ...
            # lerp(start*c, end*c, progress) == c * lerp(start, end, progress). One central change,
            # no per-adapter edits. Flat curve at 1.0 = neutral.
            if strength_curve is not None:
                cmul = _strength_curve_value(strength_curve, progress)
                cfg['high_scale_start'] *= cmul
                cfg['high_scale_end'] *= cmul
                cfg['low_scale_start'] *= cmul
                cfg['low_scale_end'] *= cmul
                cfg['_strength_curve_mul'] = float(cmul)
